...
def insert_v2_mode_section(
    ...
    mode_solver_type = mode_profile_result.get(
        "mode_solver_type",
        "approximate_te0_mode_solver_v2",
    )
    ...
    section_lines = [
        ...
        "### 1. 模块定位",
        "",
        "V2 版本在 V1.5 工程稳定版基础上，新增了近似模式求解模块。",
        "该模块用于根据 SOI 波导截面参数生成近似 TE0 模式场分布，并估算有效折射率 neff。",
        "随后，系统将该 neff 结果接回 MMI 宽度—长度二维优化流程，用于更新 MMI 初始长度和最终优化结果。",
        "",
        "需要说明的是，当前 V2 模式求解模块仍属于轻量近似模型，不是真实 FDE、FEM 或 EME 本征模求解器。",
        "其主要作用是验证“波导截面模式分析 → 有效折射率提取 → 器件参数优化”的自动化链路。",
        "",
        "### 2. 模式求解输入参数",
        "",
        "| 参数 | 数值 |",
        ...
        "### 3. 近似 TE0 模式场分布",
        "",
        "下图为系统生成的近似 TE0 模式场强分布。颜色表示归一化光强，矩形框表示波导核心区域。",
        "",
        "![近似 TE0 模式场分布](mode_profile.png)",
        "",
        "从图中可以看出，当前近似模式主要集中在波导核心附近，并向包层区域逐渐衰减。",
        ...
        "### 6. 与 MMI 优化流程的连接",
        "",
        "V2 中，MMI 优化流程不再直接使用固定经验 neff，而是读取模式求解模块输出的 neff。",
